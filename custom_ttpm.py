...
class TTPM(BaseLearner):
    ...
    def __init__(self, topology_matrix, delta=0.1, epsilon=1,
                 ...
                 true_dag=None, data_id=None, save_path="./"):
        ...
        self._true_causal_matrix = true_dag
    ...
    def learn(self, tensor, *args, **kwargs):
        ...
        # to boost up speed
        if kwargs.get("candidate_edges") is not None:
            self._candidate_edges = kwargs["candidate_edges"]
            num_can = len(self._candidate_edges)
            ratio = num_can / self._N**2
            logging.info("Given {} candidate_edges, num_event: {}, ratio: {:.3f}, {}".format(num_can, self._N, ratio, self._candidate_edges))
        else:
            self._candidate_edges = None

        # to boost up speed
        if kwargs.get("initial_dag") is not None:
            initial_edge_mat = kwargs["initial_dag"]
            initial_edge_mat = np.clip(initial_edge_mat + np.eye(self._N, self._N), 0, 1)
            logging.info("Given initial_edge_mat:")
            print(initial_edge_mat)
        else:
            initial_edge_mat = np.eye(self._N, self._N)

        # Generate causal matrix (DAG)
        _, raw_causal_matrix = self._hill_climb(initial_edge_mat)
        self._causal_matrix = pd.DataFrame(raw_causal_matrix, index=self._matrix_names, columns=self._matrix_names)
    ...
    def _k_hop_neibors(self, node, k):
        ...
        if k == 0:
            ...
            return set(nx.single_source_dijkstra_path_length(self._g, node, k).keys()) - set(nx.single_source_dijkstra_path_length(self._g, node, k - 1).keys())
    ...
    def _evaluate(self, est_causal_matrix, num_iter, likelihood, mat_change):
        # save estimated causal matrix
        filename = os.path.join(self._save_path, f"est_dag_data_{self._data_id}_iter_{num_iter}.npy")
        np.save(filename, est_causal_matrix)
        mean_edges = np.mean(est_causal_matrix)
        if self._true_causal_matrix is not None:
            # calculate g-score
            g_score = MetricsDAG(est_causal_matrix.copy(), self._true_causal_matrix).metrics['gscore']
            pair = [num_iter, likelihood, mean_edges, g_score]
        else:
            g_score = 0.0
            pair = [num_iter, likelihood, mean_edges]
        self._eval_results.append(pair)
        # save eval_results
        filename_eval = os.path.join(self._save_path, f"eval_result_{self._data_id}.npy")
        np.save(filename_eval, np.array(self._eval_results))
        logging.info("num_iter: {}, mat_change: {}, g_score: {:.2f}, likelihood: {:.1f}".format(num_iter, mat_change, g_score, likelihood))

    def _hill_climb(self, initial_edge_mat):
        """
        ...
        for num_iter in range(self._max_iter):
            ...
            if stage == "fast_check":
                ...
                candidate_edges = product(range(self._N), range(self._N))
            for new_edge_mat, op, edge in list(self._one_step_change_iterator(edge_mat, candidate_edges)):
            ...
            self._evaluate(edge_mat, num_iter, l_ret, mat_change)

            if stop_tag:
                if stage == "fast_check":
                    stage = "full_check"
                    print("#### change to stage:", stage)
                else:
    ...
    def _em(self, edge_mat):
        ...
        causal_g = nx.from_numpy_matrix((edge_mat - np.eye(self._N, self._N)), create_using=nx.DiGraph)
    ...
    def _one_step_change_iterator(self, edge_mat, candidate_edges):

        return map(lambda e: (*self._one_step_change(edge_mat, e), e), candidate_edges)

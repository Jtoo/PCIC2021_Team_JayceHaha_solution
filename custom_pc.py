...
class PC(BaseLearner):
    ...
    def __init__(self, alpha=0.05, ci_test='gauss', max_k=None):
    ...
    def learn(self, data, **kwargs):
        ...
        skeleton, sep_set = FindSkeleton.origin_pc(data=data, alpha=self.alpha,
                                                   ci_test=self.ci_test, max_k=self.max_k)
        print("#### PC algo, done finding skeleton:")
        print(skeleton)
...
class FindSkeleton(object):
    ...
    @staticmethod
    def origin_pc(data, alpha=0.05, ci_test='gauss', max_k=None):
        ...
        while k <= n_features - 2:
            ...
            for i, j in combinations(nodes, 2):
                if k == 0:
                    if ci_test == 'gauss':
                        print(f"#### gauss ci_test, k: {k}, i-j: {i}, {j}")
                    ...
                    for ctrl_var in combinations(other_nodes, k):
                        ...
                        if ci_test == 'gauss':
                            print(f"#### gauss ci_test, k: {k}, i-j: {i}, {j}, ctrl_var: {ctrl_var}")
                            p_value = CI_Test.gauss_test(data, i, j, ctrl_var)
                        else:
                            raise ValueError('Unknown ci_test method, please check '
                                             f'the parameter {ci_test}.')
                        ...
                        if s:
                            skeleton[i, j] = skeleton[j, i] = 0
                            sep_set[(i, j)] = s
                            print(f"#### add d-sep_set, k: {k}, i-j: {i}, {j}, ctrl_var: {ctrl_var}")
                            break
            k += 1

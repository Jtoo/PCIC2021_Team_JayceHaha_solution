...
class CI_Test(object):
    ...
    def gauss_test(data, x, y, ctrl_var):
        ...
        if k == 0:
            ...
            # selected_data = data[:, sub_index].T
            # selected_data_dirty = selected_data + 0.00001 * np.random.uniform(0, 1.0, size=selected_data.shape)
            # sub_corr = np.corrcoef(selected_data_dirty)
            # print(sub_corr)
            # print(np.linalg.svd(sub_corr)[1])
            # inverse matrix
